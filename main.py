...
def extract_president_names(directory):
...
def list_presidents_names(directory):
    ...
    if not os.path.exists(directory):
      ...
      president_names = ", ".join(set(extract_president_names(directory).split("\n")))
      message= f"Here's the list of president (without duplicates) available in our data base : {president_names}"
    return message



def convert_text_cleaned(filename, cleaned_directory):
    """
    ...
    if not os.path.exists(os.path.join("speeches", filename)):
        message= f"There is no file named '{filename}' in speeches ⚠ "
    else:
...
def convert_folder_cleaned(directory,cleaned_directory):
    ...
    if not os.path.exists(directory):
        message= f"You don't have any speeches in '{directory}' ⚠ "
    else:       
      # Read files from the 'speeches' folder
      for filename in os.listdir(directory):
          filepath = os.path.join(directory, filename)
          convert_text_cleaned(filename, cleaned_directory)
      ...
      if president_name in list_presidents_names(directory):
          ...
          for filename in os.listdir(directory):
            ...
            if filename.startswith(f"Nomination_{president_name}"):
                ...
                tf_scores = tf_score(filename[:-4] + "_removed_punctuation.txt", cleaned_directory)  # Calculate TF scores
...
def presidents_who_mentioned (directory,cleaned_directory, chosen_word):
    ...
    if not os.path.exists(cleaned_directory):
      ...
      for filename in os.listdir(directory):
        ...
        tf_scores = tf_score(filename[:-4] + "_removed_punctuation.txt", cleaned_directory)
      ...
      if chosen_word_dict == {}:
        ...
        message = f"The word '{chosen_word}' has been mentioned by {list_president}. But, {repeated_it_the_most} mentioned it the most, he repeated it {frequency} times."
    return message
...
def first_president_to_mention(cleaned_directory, chosen_word, matrix=None):
    ...
    if not os.path.exists(cleaned_directory):
      ...
      if mention_indices==[]:
          message= f"The theme '{chosen_word}' hasn't been mentioned by any president available in our database ⚠ "
      else:
          ...
          message= f"The theme '{chosen_word}' was first mentioned by {president_name.split('_')[1]} in {nomination_year}. Words used : {filtered_words}"
    return message
...
print(convert_text_cleaned("Nomination_Julien.txt", "cleaned"))
print(convert_folder_cleaned("speeches","cleaned"))
print("")


# Call of the function remove_punctuation()
print(remove_punctuation_text("Nomination_Timothée.txt", "cleaned"))
print(remove_punctuation_folder("cleaned"))
print("")

# Call of the function tf_score()
(tf_score("Nomination_Macron_removed_punctuation.txt", "cleaned"))
print(tf_score("Nomination_Julien_removed_punctuation.txt", "cleaned"))

# Call of the function idf_score()
(idf_score("cleaned"))
print(idf_score("removed_punctuation"))
print("")
...
# Call of the function most_repeated_word_by_president()
print(most_repeated_word_by_president("speeches","Chirac","cleaned",least_important_included=False,matrix=matrix))
print(most_repeated_word_by_president("speeches","Chirac","cleaned",True,matrix))
print(most_repeated_word_by_president("speeches","Timothée","cleaned",least_important_included=False,matrix=matrix))
print("")
...
# Call of the first_president_to_mentiont()
print(first_president_to_mention("cleaned","climat", matrix))
print(first_president_to_mention("cleaned","ecologie", matrix))
print(first_president_to_mention("cleaned","economie", matrix))
